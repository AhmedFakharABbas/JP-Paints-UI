...
@Component({
  selector: 'app-workorder-other',
  templateUrl: './workorder-other.component.html',
  styleUrls: ['./workorder-other.component.css']
})
export class WorkorderOtherComponent implements OnInit {
  ...
  onGetEstimate(id: number) {
    this._adminsideService.getEstimate(id).subscribe((res: any) => {
      ...
      if (this.projectDetailsObj.project_information.project_type_id == 4) {
        this.generatedPdfName = "other" + this.project_id + "-work" + ".pdf";
      }

      if (this.projectDetailsObj.project_information.project_type_id == 5) {
        this.generatedPdfName = "klean_ups" + this.project_id + "-work" + ".pdf";
      }

      if (this.projectDetailsObj.project_information.project_type_id == 6) {
        this.generatedPdfName = "empowered" + this.project_id + "-work" + ".pdf";
      }
